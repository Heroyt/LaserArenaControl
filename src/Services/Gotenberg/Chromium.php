...
class Chromium
{
	...
	public function getFromUrl(string $url, array $data = []) : string {
		...
		if (!isset($response) || !$this->isResponseValid($response)) {
			$this->service->getLogger()->warning('Invalid response '.json_encode([
				                                                                     'isset' => isset($response),
				                                                                     'code'  => $response->getStatusCode() === 200,
				                                                                     'type'  => str_contains($response->getHeaderLine('Content-Type'),
				                                                                                             'application/pdf'),
			                                                                     ]));
	...
	protected function isResponseValid(ResponseInterface $response) : bool {
		return $response->getStatusCode() === 200 && str_contains($response->getHeaderLine('Content-Type'),
		                                                          'application/pdf');
	}
