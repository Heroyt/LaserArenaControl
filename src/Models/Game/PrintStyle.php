...
class PrintStyle extends AbstractModel
{
	...
	public function getCssClasses(bool $tag = true) : string {
		...
		foreach (self::COLORS as $color) {
			$return .= '.print-'.$color.' {background-color: var(--print-'.$color.'); color: var(--print-'.$color.'-text);}';
			$return .= '.bg-print-'.$color.' {background-color: var(--print-'.$color.');}';
			$return .= '.text-print-'.$color.' {color: var(--print-'.$color.');}';
		}
		if ($tag) {
			$return .= '</style>';
		}
